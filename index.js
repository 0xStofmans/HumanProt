...
async function claimRewards(privateKey, web3, contract,rpcUrl) {
    try {
        //const wallet = new ethers.Wallet(privateKey, provider);
        const account = new ethers.Wallet(privateKey, rpcUrl) //web3.eth.accounts.privateKeyToAccount(privateKey);
...
async function main() {
    ...
    // Loop every 6 hours
    while (true) {
        //const privateKeys = await readWallets(); //loadPrivateKeys(config.privateKeysFile || './private_keys.txt'); // Specify file path or use default
        const privateKeys = JSON.parse(fs.readFileSync('privateKeys.json'));
        for (const privateKey of privateKeys) {
            await claimRewards(privateKey, web3, contract,rpcUrl);
        }

        console.log(chalk.cyan('Waiting 24 hours before running again...'));
        await new Promise(resolve => setTimeout(resolve, 24 * 60 * 60 * 1000)); // Wait 6 hours
    }
}
